...
const setHoveredQuadOrHorizon = radarModule.setPayloadAction<
  QuadOrOrizonOptsType | Array<QuadOrOrizonOptsType> | null
>(ActionType.SET_HOVERED_QUADRANT, (state, action) => ({
  ...state,
  hoveredQuadOrHorizon: action.payload
}));
...
const setRadarIsReady = radarModule.setPayloadAction<boolean>(
  ActionType.SET_RADAR_IS_READY,
  (state, action) => ({ ...state, radarIsReady: action.payload })
);
